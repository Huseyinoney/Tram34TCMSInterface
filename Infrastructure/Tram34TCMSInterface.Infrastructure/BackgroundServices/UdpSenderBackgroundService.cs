...
public class UdpSenderBackgroundService : BackgroundService
{
    ...
    private bool _cabAActive;
    private bool _cabAKeyStatus;
    ...
    public UdpSenderBackgroundService(IConfiguration configuration)
    {
        ...
        // Başlangıç değerleri IConfiguration'dan alınıyor
        _cabAActive = Convert.ToBoolean(configuration["TrainSettings:Cab_A_Active"]);
        _cabAKeyStatus = Convert.ToBoolean(configuration["TrainSettings:Cab_A_KeyStatus"]);
    }
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            bool tachoMeterPulse, zeroSpeed, doorsOpen, cabAActive, cabAKeyStatus;
            double trainSpeed;
            string[] coupledTrains;
            ...
            lock (_lock)
            {
                ...
                doorsOpen = _doorsOpen;
                coupledTrains = _coupledTrains.ToArray();
                cabAActive = _cabAActive;
                cabAKeyStatus = _cabAKeyStatus;
            }
            ...
            var train = new
            {
                ID = configuration["TrainSettings:ID"],
                IP = configuration["TrainSettings:IP"],
                Cab_A_Active = cabAActive,
                Cab_B_Active = Convert.ToBoolean(configuration["TrainSettings:Cab_B_Active"]),
                Cab_A_KeyStatus = cabAKeyStatus,
                Cab_B_KeyStatus = Convert.ToBoolean(configuration["TrainSettings:Cab_B_KeyStatus"]),
                TrainCoupledOrder = Convert.ToInt32(configuration["TrainSettings:TrainCoupledOrder"]),
                IsTrainCoupled = coupledTrains.Length > 0,
                AllDoorOpen = doorsOpen,
    ...
    private async void MonitorKeyPress()
    {
        while (true)
        {
            ...
            // B tuşu: Cab_A_Active ve Cab_A_KeyStatus tersle
            if (IsKeyDown(ConsoleKey.B))
            {
                lock (_lock)
                {
                    _cabAActive = !_cabAActive;
                    _cabAKeyStatus = !_cabAKeyStatus;
                    Console.WriteLine($"B tuşuna basıldı: Cab_A_Active ve Cab_A_KeyStatus {_cabAActive}");
                }
