...
namespace Tram34TCMSInterface.Infrastructure.Services.UDP
{
    public class ReadDataFromTCMSWithUDP : IReadDataFromTCMSWithUDP
    {
        ...
        private bool AreCoupledTrainIdsEqual(List<string> currentIds, List<string> previousIds)
        {
            ...
            var currentSorted = currentIds.OrderBy(x => x).ToList();
            var previousSorted = previousIds.OrderBy(x => x).ToList();

            return currentSorted.SequenceEqual(previousSorted);
        }
        ...
        //                        mongoDBTrainConfigurationCacheService.SaveTrainInformationToCache(currentTrain.ID);
        //                        logService.TrainId = currentTrain.ID;
        //                         logService.SendLogAsync<EventLog>(logFactory.CreateEventLog("Kuplaj Bilgisi TCMS'ten Alındı","TCMSInterface","","",""));
        //                    }
